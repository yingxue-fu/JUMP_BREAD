...
batch_shift_adding <- function(initial_batch_design_matrix, n_samples_f, n_samples_batches) {
  ...
  if (sum(abs(n_incosis_row)) !=0) {
    ...
    from_batch_index = names(n_incosis_row)[n_incosis_row < 0] 
    to_batch_index = names(n_incosis_row)[n_incosis_row > 0] 
    
    n_from_batch = length(from_batch_index)
    n_to_batch = length(to_batch_index)
    n_samples_f_sort <- sort.int(n_samples_f, index.return=T, decreasing=T,method = "quick")
    
    for (i in 1:n_from_batch) {
      for (j in 1:n_to_batch) {
        n_switch_samples <- n_incosis_row[to_batch_index[j]]
        from_group_index <- n_samples_f_sort$ix[i]  
        new_batch_design_matrix[from_batch_index[i], from_group_index] <- new_batch_design_matrix[from_batch_index[i], from_group_index] - n_switch_samples 
        new_batch_design_matrix[to_batch_index[j], from_group_index] <- new_batch_design_matrix[to_batch_index[j], from_group_index] + n_switch_samples 
      }
    }
  }
...
block_rand_one_factor <- function(sample_factors_df, tmtPlex, useIR, IRchannel, n_iterations = 100) {
  ...
  if (ncol(sample_factors_df) < 2) {
    ...
    if (useIR == "No") {
      batch_volume <- tmtPlex
    } else if (useIR == "Yes") {
      batch_volume <- tmtPlex - length(IRchannel)
    }
...
block_rand_two_factors <- function(sample_factors_df, tmtPlex, useIR, IRchannel, n_iterations = 100) {
  # sample_factors_df: a dataframe, first column should be sample ID
  # The second to last columns should be the factors that need randomization 
...
block_rand_3orMore_factors <- function(sample_factors_df, n_factors, tmtPlex, useIR, IRchannel, n_iterations = 100) {
  # sample_factors_df: a dataframe, first column should be sample ID
  # The second to last columns should be the factors that need randomization 
...
# add TMT channels
add_TMT_channels <- function(sample_batch_final, useIR, IRchannel, tmtPlex, n_batches, n_factors) {
  
  # extract used TMT channels based on # of TMTplex
  used_tmt_channels <- tmt_channels[1:tmtPlex]
  
  # extract TMT channels used for samples
  if (useIR == "No") {
    tmt_channels_used_for_samples <- used_tmt_channels
  } else if (useIR == "Yes") {
    tmt_channels_used_for_samples <- used_tmt_channels[!(used_tmt_channels %in% IRchannel)]
  }
  ...
  if (useIR == "No") {
    ...
    n_IR_batch = length(IRchannel)
    IR_samples <- data.frame(matrix(nrow = n_batches*n_IR_batch, ncol = n_factors + 3))
    colnames(IR_samples) <- colnames(sample_batch_final)
    IR_samples[,1] <- "IR"
    IR_samples$batchID <- rep(batchID, each = n_IR_batch)
    IR_samples$TMT_channel <- rep(IRchannel, n_batches)
...
final_outputs <- function(sample_factors_df, n_factors, tmtPlex, useIR, IRchannel, n_iterations = 100) {
  
  # batch assign
  if (n_factors == 1) {
    sample_batch_final <- block_rand_one_factor(sample_factors_df, tmtPlex, useIR, IRchannel, n_iterations)
  } else if (n_factors == 2) {
    sample_batch_final <- block_rand_two_factors(sample_factors_df, tmtPlex, useIR, IRchannel, n_iterations)
  } else if (n_factors >= 3) {
    sample_batch_final <- block_rand_3orMore_factors(sample_factors_df, n_factors, tmtPlex, useIR, IRchannel, n_iterations)
  }
  
  n_batches <- length(unique(sample_batch_final$batchID))
  # add other and IR samples
  sample_batch_final_add <- add_TMT_channels(sample_batch_final, useIR, IRchannel, tmtPlex, n_batches, n_factors)
  
  # sort by batch ID and TMT channels
  sample_batch_final_add$TMT_channel = factor(sample_batch_final_add$TMT_channel, levels = tmt_channels[1:tmtPlex])
  ...
  for (i in 1:n_factors) {
    batch_factor_table <- table(sample_batch_final_add$batchID, sample_batch_final_add[,(i+2)]) %>% as.data.frame.matrix() 
    batch_design_factos[[i]] <- batch_factor_table[stringr::str_order(rownames(batch_factor_table), numeric = T), ]
    tmt_factor_table <- table(sample_batch_final_add$TMT_channel, sample_batch_final_add[,(i+2)]) %>% as.data.frame.matrix()
